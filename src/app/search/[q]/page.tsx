...
export async function generateMetadata({ params }: { params: { q: string } }): Promise<Metadata> {
	try {
		const res = await fetch(`${process.env.NEXT_PUBLIC_VERCEL_DOMAIN}/api/weather?q=${params.q}&key=${process.env.NEXT_PUBLIC_WEATHER_API_KEY}`);
...
export default async function SearchPage({ params }: { params: { q: string } }) {
	try {
		const WeatherDataRequest: WeatherData = await getWeatherData({ q: params.q });
